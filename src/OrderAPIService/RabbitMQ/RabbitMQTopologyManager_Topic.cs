...
public class RabbitMQTopologyManager_Topic
{
    private readonly RabbitMQConnection _rabbitMQConnection;


    public RabbitMQTopologyManager_Topic(RabbitMQConnection rabbitMQConnection)
    {
    ...
    public async Task CreateTopologyAsync()
    {
        ...
        await channel.ExchangeDeclareAsync(exchange: RabbitMQConstants.DeadLetterExchange, type: ExchangeType.Fanout, durable: true, autoDelete: false);
        Console.WriteLine("email.dlx created successfully.");
        //await channel.ExchangeDeclareAsync(exchange: RabbitMQConstants.DeadLetterExchange, type: ExchangeType.Direct, durable: true, autoDelete: false);
        ...
        var retryQueueArguments = new Dictionary<string, object?>
        {
            ...
            { "x-dead-letter-routing-key", RabbitMQConstants.OrderCreatedRoutingemail },
        };
        ...
        await channel.QueueBindAsync(queue: "email.deadletter.queue", exchange: RabbitMQConstants.DeadLetterExchange, routingKey: "email.deadletter");


        await channel.QueueBindAsync(queue: RabbitMQConstants.EmailQueue, exchange: RabbitMQConstants.NotificationExchange, routingKey: RabbitMQConstants.OrderCreatedRoutingStar);
        Console.WriteLine("email.queue bound to notification.exchange.");

        // await channel.QueueBindAsync(queue: RabbitMQConstants.SmsQueue, exchange: RabbitMQConstants.NotificationExchange, routingKey: RabbitMQConstants.OrderCreatedRoutingStar);
